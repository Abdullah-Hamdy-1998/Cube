                                ...
                                <div class="d-flex">
                                    <div class="btn-group mr-2" role="group" aria-label="First group">
                                        <button type="button" class="btn btn-primary data_btn mr-2 border-radius-coust"
                                            data-id="{{ $supplier->id }}" data-route="supplier-restore"><i
                                                class="eva eva-trash-outline"></i></button>
                                    </div>
                                    <div class="d-inline">
                                        @can('write-trash')
                                            <button class="btn btn-danger data_btn border-radius-coust"
                                                data-id="{{ $supplier->id }}" data-route="supplier-delete"><i
...
@section('jquery')
    <script>
        $(document).on('click', '.btn', function() {
            ...
            if (route == 'supplier-delete') {
                method = 'DELETE';
            } else if (route == 'supplier-restore') {
                method = 'POST';
            }
